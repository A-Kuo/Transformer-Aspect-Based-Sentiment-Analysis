...
Amazon reviews skew heavily positive (most ratings are 4–5 stars), which without correction collapses training into mostly predicting "positive" regardless of input — this is why the per-class sentiment recall floors exist separately from overall accuracy, and why `training.class_weighted_loss` (`src/data.py::compute_class_weights`, sklearn's inverse-frequency "balanced" heuristic) is on by default. Full inverse-frequency weighting can itself overcorrect under severe imbalance and limited training, collapsing predictions onto whichever class the weighting now favors most (e.g. "neutral", the rarest sentiment class) instead of fixing the skew — `training.class_weight_power` (default `0.5`, sqrt-dampened) softens this correction; see `compute_class_weights`'s docstring for the exact formula. Aspect labels have a separate, unrelated data-quality issue: `assign_aspect` uses word-boundary regex matching (not naive substring matching) specifically to avoid false positives like "late" inside "chocolate" or "box" inside "boxed" inflating a keyword category's (e.g. "shipping") share of the weak-supervision labels.
